...
module.exports.welcome = async(iswel, isleft, chiwa, anu) =>{
	try {
            const metadata = await chiwa.groupMetadata(anu.id)
            ...
            for (let num of participants) {
                ...
                try {
                    pp_user = await chiwa.profilePictureUrl(num, 'image')
                } catch {
                    pp_user = 'https://telegra.ph/file/c3f3d2c2548cbefef1604.jpg'
                }

                try {
                    ppgroup = await chiwa.profilePictureUrl(anu.id, 'image')
                } catch {
...
try {
	var welcome = await getBuffer(`https://api.lolhuman.xyz/api/base/welcome?apikey=chiwakawa&img1=${ppu}&img2=${ppgc}&background=https://i.ibb.co/8B6Q84n/LTqHsfYS.jpg&username=${encodeURI(num.split("@")[0])}&member=${encodeURI(memeg)}&groupname=${encodeURI(metadata.subject)}`)
  // https://web-welcome-zeeoneofficial.cloud.okteto.net/api/welcome2?name=${encodeURI(num.split("@")[0])}&mem=${encodeURI(memeg)}&gcname=${encodeURI(metadata.subject)}&picurl=${ppu}&desc=baca&bgurl=https://telegra.ph/file/90a931648de597820bc08.jpg&gcicon=${ppgc}
    ...
    chiwa.sendMessage(anu.id, {image: welcome, caption: full_pesan, mentions: [num]}, {quoted: fkontaku})
                      // chiwa.sendMessage(anu.id, { caption: `${full_pesan}`, image: { url: pp_user }, mentions: [num] })
                   } else {
chiwa.sendMessage(anu.id, {image: welcome, caption: `Welcome @${num.split("@")[0]} in the group ${groupName}\n${groupDesc}`, mentions: [num]}, {quoted: fkontaku})
                      }
...
try {
	var leave = await getBuffer(`https://api.lolhuman.xyz/api/base/leave?apikey=chiwakawa&img1=${ppu}&img2=${ppgc}&background=https://i.ibb.co/8B6Q84n/LTqHsfYS.jpg&username=${encodeURI(num.split("@")[0])}&member=${encodeURI(memeg)}&groupname=${encodeURI(metadata.subject)}`)
        } catch {
...
chiwa.sendMessage(anu.id, {image: leave, caption: full_pesan, mentions: [num]}, {quoted: fkontaku})

                       } else {
chiwa.sendMessage(anu.id, {image: leave, caption: `Sayonara @${num.split("@")[0]}`, mentions: [num]}, {quoted: fkontaku})
                        } 
                } else if (anu.action == 'promote') {
                   // chiwa.sendMessage(anu.id, { image: { url: pp_user }, mentions: [num], caption: `@${num.split('@')[0]} Promote In ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    //chiwa.sendMessage(anu.id, { image: { url: pp_user }, mentions: [num], caption: `@${num.split('@')[0]} Demote In ${metadata.subject}` })
              }
}
        } catch (err) {
            console.log(err)
	...
	module.exports.aDelete = async (setting, chiwa, m) => {
...
✍️ *MessageType* : ${mek.mtype}
    `,
		...
		await chiwa.copyNForward(froms, mek, true);
  	await delay(4000)
...
module.exports.oneTime = async (setting,chiwa,m) => {
